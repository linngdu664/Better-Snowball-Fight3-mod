    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (!isCaught && pResult.getEntity() instanceof LivingEntity livingEntity && targetLocator != null) {
            ...
            if (getOwner() instanceof Player player) {
                player.displayClientMessage(new TranslatableComponent("target.tip").append(livingEntity.getName().getString() + " UUID:" + livingEntity.getUUID()), false);
                if (pResult.getEntity() instanceof Player player1) {
                    player1.displayClientMessage(new TranslatableComponent("targeted.tip"), false);
                }
            ...
            targetLocator.setHoverName(new TranslatableComponent("item.bsf.target_locator").append(":").append(new TranslatableComponent("target.tip").append(livingEntity.getName().getString() + " UUID:" + livingEntity.getUUID())));
        }
    }
