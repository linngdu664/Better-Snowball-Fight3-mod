...
//        if (!storageInTank(pPlayer, itemStack, ItemRegister.STONE_SNOWBALL_TANK.get())) {
//            pLevel.playSound(null, pPlayer.getX(), pPlayer.getY(), pPlayer.getZ(), SoundEvents.SNOWBALL_THROW, SoundSource.NEUTRAL, 0.5F, 0.4F / (pLevel.getRandom().nextFloat() * 0.4F + 0.8F));
//            if (!pLevel.isClientSide) {
//                StoneSnowballEntity snowballEntity = new StoneSnowballEntity(pPlayer, pLevel, getLaunchFunc(getSnowballDamageRate(pPlayer)));
...
//        return InteractionResultHolder.sidedSuccess(itemStack, pLevel.isClientSide());
    ...
    public BSFSnowballEntity getCorrespondingEntity(Level level, LivingEntity livingEntity, LaunchFunc launchFunc) {
        return new StoneSnowballEntity(livingEntity, level, launchFunc);
    }
    ...
    public void appendHoverText(@NotNull ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, @NotNull TooltipFlag pIsAdvanced) {
        ...
        pTooltipComponents.add(MutableComponent.create(new TranslatableContents("stone_snowball.tooltip", null, new Object[0])).withStyle(ChatFormatting.DARK_AQUA));
    }
}
