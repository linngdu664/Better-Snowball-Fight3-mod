...
public class BasinItem extends Item {
    ...
    public @NotNull Component getName(ItemStack pStack) {
        ...
        if (compoundTag.contains("SnowType")) {
            if (compoundTag.getByte("SnowType") == 1) {
                return new TranslatableComponent("item.bsf.basin_of_snow");
            }
            return new TranslatableComponent("item.bsf.basin_of_powder_snow");
        }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, @NotNull List<Component> pTooltipComponents, @NotNull TooltipFlag pIsAdvanced) {
        ...
        if (!tag.contains("SnowType")) {
            pTooltipComponents.add(new TranslatableComponent("empty_basin.tooltip1").withStyle(ChatFormatting.BLUE));
    ...
    public boolean isNotBlocked(Vec3 rVec, Vec3 rVec1, Player player, Level level) {
        double offsetX = 0.25 * rVec.z * Mth.fastInvSqrt(BSFCommonUtil.lengthSqr(rVec.x, rVec.z));
        double offsetZ = 0.25 * rVec.x * Mth.fastInvSqrt(BSFCommonUtil.lengthSqr(rVec.x, rVec.z));
