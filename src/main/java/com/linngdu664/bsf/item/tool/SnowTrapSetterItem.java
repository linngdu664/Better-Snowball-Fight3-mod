...
public class SnowTrapSetterItem extends AbstractBSFEnhanceableToolItem {
    ...
    public void onUseTick(@NotNull Level pLevel, @NotNull LivingEntity pLivingEntity, @NotNull ItemStack pStack, int pRemainingUseDuration) {
        ...
        if (!pLevel.getBlockState(blockPos).getBlock().equals(Blocks.SNOW)) {
            ...
            NetworkRegister.PACKET_HANDLER.send(PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> player), new ForwardRaysParticlesToClient(centerPos.add(-0.5, -0.4, -0.5), centerPos.add(0.5, -0.4, 0.5), new Vec3(0, 1, 0), 0.1, 0.3, 5));
    ...
    public void appendHoverText(@NotNull ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, @NotNull TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(new TranslatableComponent("snow_trap_setter.tooltip").withStyle(ChatFormatting.DARK_AQUA));
    }
}
