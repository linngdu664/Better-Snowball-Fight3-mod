                ...
                if (cosAlpha > 0.17) {
                    ...
                    if (r < x && r * sinAlpha < Math.sqrt(BSFCommonUtil.lengthSqr(aabb.maxX - aabb.minX, aabb.maxZ - aabb.minZ)) * 0.5 + 0.8) {
                        ...
                        if (y >= aabb.minY - 0.8 && y <= aabb.maxY + 0.8) {
                            if (entity.getType().equals(golem.getTarget().getType()) && !entity.equals(golem.getOwner()) && !golem.isUseLocator()) {
        ...
        for (int r = 4; r <= 10; r++) {
            ...
            for (float theta = 0; theta < Mth.PI * 0.25; theta += step) {
                for (float phi = 0; phi < Mth.PI * 0.5; phi += step) {
                    ...
                    if (golem.canStandOn(blockPos, level) && level.clip(new ClipContext(Vec3.atCenterOf(blockPos.above()), targetPos, ClipContext.Block.COLLIDER, ClipContext.Fluid.ANY, golem)).getBlockPos().equals(targetBlockPos)) {
