...
public class BSFGolemJumpHighGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (target != null && golem.distanceToSqr(target) < 9 && (golem.getStatus() == 2 || golem.getStatus() == 3)) {
            ...
            if (maxJ != 12) {
                ...
                pushVec = new Vec3(Mth.cos(theta) * Mth.cos(phi), Mth.sin(phi), Mth.sin(theta) * Mth.cos(phi)).scale(1.75);
                return true;
            }
        }
        return false;
    ...
    @Override
    public void start() {
        golem.setDeltaMovement(pushVec);
    }
